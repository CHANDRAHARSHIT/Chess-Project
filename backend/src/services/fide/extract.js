...
// ─── Main ──────────────────────────────────────────────────────────
export function extractFideGames(fideId) {
    // 1. Resolve player
    ...
    for (const filename of pgnFiles) {
        ...
        for (const gamePgn of games) {
            ...
            const playerResult = matchPlayer(headers, player.fide_id, player.aliases);
            ...
            const filterResult = classicalFilter(timeControl, eventName, siteName);
            ...
            if (filterResult.method === 'time_control')
                passedByTc++;
            else if (filterResult.method === 'event_include')
                passedByEventInclude++;
            else if (filterResult.method === 'event_default')
...
if (isMain || process.argv[1].endsWith('extract.ts')) {
    main();
}
