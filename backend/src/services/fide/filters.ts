...
export interface ClassicalFilterResult {
  passed: boolean;
  reason: string;
  method: 'time_control' | 'event_include' | 'event_exclude' | 'event_default';
}
 ...
 */
function detectEventNameTC(eventName: string): { minutes: number; increment: number } | null {
 ...
 */
function isOnlinePlatform(eventName: string, siteName: string): string | null {
 ...
 */
export function classicalFilter(
  timeControl: string | null | undefined,
  eventName: string,
  siteName: string = ''
): ClassicalFilterResult {
