...
import { Search, Globe, ChevronDown, CheckSquare, Settings } from "lucide-react";
...
export default function DatabasePlayerPage() {
  ...
  return (
    <div className="min-h-screen bg-[#080B14] text-[#F5F0E8] font-sans p-6 md:p-10 lg:p-12 overflow-y-auto w-full relative">
        ...
        <div className="flex-1 w-full">
          ...
          <div className="flex flex-col md:flex-row gap-6 mb-10 items-center md:items-start">
            ...
            <div className="flex md:hidden items-center gap-3 w-full justify-center">
              <span className="bg-red-600 text-white font-bold px-1.5 py-0.5 rounded text-xs">GM</span>
            ...
            <div className="flex flex-col justify-center w-full">
              ...
              <table className="text-sm text-[#8E8B82] border-separate border-spacing-y-1 w-full md:w-auto mt-4 md:mt-0">
                <tbody>
                  <tr>
                    <td className="font-semibold text-[#D4AF6E] pr-4">Full name</td>
                    <td>Garry Kimovich Kasparov</td>
                  </tr>
                  <tr>
                    <td className="font-semibold text-[#D4AF6E] pr-4">Born</td>
                    <td>Apr 13, 1963 (age 63)</td>
                  </tr>
                  <tr>
                    <td className="font-semibold text-[#D4AF6E] pr-4">Place of birth</td>
                    <td>Baku, Azerbaijan SSR, Soviet Union</td>
                  </tr>
                  <tr>
                    <td className="font-semibold text-[#D4AF6E] pr-4">Federation</td>
                    <td>Russia</td>
                  </tr>
                </tbody>
              </table>
          ...
          <div>
            <div className="flex items-center justify-between mb-4 border-b border-brand-border/40 pb-2">
              <h2 className="text-xl font-display font-semibold text-[#F5F0E8]">Garry Kasparov Chess Games</h2>
              <div className="relative">
                <button 
                  onClick={() => setIsSortOpen(!isSortOpen)}
                  className="text-sm text-[#8E8B82] hover:text-[#D4AF6E] flex items-center gap-1 transition-colors font-sans"
                >
                ...
                {isSortOpen && (
                  <div className="absolute right-0 top-6 w-48 bg-[#0C1020] border border-brand-border/40 rounded shadow-2xl z-50 overflow-hidden font-sans">
                    {[
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-sm transition-colors ${sortOption === option.value ? 'bg-brand-accent/20 text-[#D4AF6E]' : 'text-[#8E8B82] hover:bg-white/5 hover:text-[#F5F0E8]'}`}
                      >
                        {option.label}
                      </button>
                    ))}
            ...
            <table className="w-full text-left text-sm">
              <thead className="text-[#8E8B82] text-xs uppercase tracking-wider">
                <tr>
              ...
              <tbody className="text-[#F5F0E8] divide-y divide-brand-border/20">
                  ...
                  >
                    <td className="py-3">
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center text-xs text-[#8E8B82] mt-1">
                          <span className="truncate">{game.pgn.split(" 4. ")[0]}</span>
                          <span className="mx-1.5 opacity-50 shrink-0">|</span>
                          <span className="text-[#D4AF6E] shrink-0">{game.opening}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 font-mono text-[#D4AF6E] text-center">{game.result}</td>
                    <td className="py-3 text-[#8E8B82] text-center">{game.moves}</td>
                    <td className="py-3 text-[#8E8B82] hidden md:table-cell text-center">{game.year}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="w-full lg:w-80 flex-shrink-0 flex flex-col items-center gap-6">
          <div className="w-full rounded-xl border border-brand-border bg-[#0C1020] p-5 shadow-2xl opacity-40 pointer-events-none">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold font-display text-[#F5F0E8]">Games</h2>
              <ChevronDown className="w-4 h-4 text-[#8E8B82]" />
            </div>
            <p className="text-xs text-[#8E8B82] mb-3">Select an opening or player to search</p>
            
            <div className="space-y-3">
              <div className="relative flex items-center gap-2 bg-[#0A0E1A] border border-brand-border/60 rounded-lg pr-2 focus-within:border-brand-accent/50 transition-colors">
                <Search className="ml-3 w-4 h-4 text-[#8E8B82]" />
                <input 
                  type="text" 
                  placeholder="Opening"
                  className="w-full bg-transparent py-2 text-sm text-[#F5F0E8] focus:outline-none"
                />
              </div>
              ...
              <div className="relative flex items-center gap-2 bg-[#0A0E1A] border border-brand-border/60 rounded-lg pr-2 focus-within:border-brand-accent/50 transition-colors">
                ...
                <input 
                  ...
                  readOnly
                />
                <button className="text-[#8E8B82] hover:text-[#F5F0E8]"><Settings className="w-3 h-3"/></button>
              </div>
      ...
      {hoveredGame && (
        <div 
          className="fixed z-50 p-2 bg-[#080B14] rounded-lg border border-brand-border/60 shadow-2xl pointer-events-none transition-opacity duration-200"
...
function StatBar({ label, win, draw, loss }: { label: string, win: number, draw: number, loss: number }) {
  return (
    <div>
      <div className="flex justify-between text-sm font-bold text-[#F5F0E8] mb-1.5">
        <span>{label}</span>
      </div>
      <div className="w-full h-5 flex overflow-hidden text-xs font-bold text-white text-center">
        <div style={{ width: `${win}%`, minWidth: win > 0 ? '80px' : undefined }} className="bg-[#7FA650] flex items-center justify-center border-r border-[#080B14]/50 relative">
          <span className="whitespace-nowrap z-10 drop-shadow-sm">{win > 0 ? `${win}% Win` : ''}</span>
        </div>
        <div style={{ width: `${draw}%`, minWidth: draw > 0 ? '80px' : undefined }} className="bg-[#8E8B82] flex items-center justify-center border-r border-[#080B14]/50 relative">
